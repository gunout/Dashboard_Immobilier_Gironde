...
st.sidebar.header("Selection de la commune")
...
st.info(f"Donnees DVF pour **{selected_commune_name}** (INSEE {selected_insee_code})")

with st.spinner("Chargement..."):
    all_data = load_all_data()

if all_data.empty:
    st.warning("Aucune donnee valide.")
    st.stop()

with st.sidebar.expander("Diagnostic"):
    if "code_commune" in all_data.columns:
        st.write(f"Code recherche : {selected_insee_code}")
        present = selected_insee_code in all_data["code_commune"].values
        st.write(f"Trouve : {'OUI' if present else 'NON'}")
        if not present:
            st.write("Exemples codes presents :")
            st.write(sorted(all_data["code_commune"].unique())[:30])

df = all_data[all_data['code_commune'] == selected_insee_code].copy()

if df.empty:
    st.warning(f"Aucune donnee pour {selected_commune_name} (code {selected_insee_code}).")
    st.stop()
...
col1.metric("Prix/m2 moyen", f"{df_filtre['prix_m2'].mean():.0f} EUR")
col2.metric("Prix median", f"{df_filtre['valeur_fonciere'].median():.0f} EUR")
col3.metric("Transactions", f"{len(df_filtre):,}")
col4.metric("Surface moyenne", f"{df_filtre['surface_reelle_bati'].mean():.0f} m2")
...
st.subheader(f"Carte des Transactions a {selected_commune_name}")
...
if 'latitude' in df_filtre.columns and 'longitude' in df_filtre.columns:
    ...
    if valid.any():
        carte = df_carte[valid].sample(min(2000, len(df_carte)), random_state=42)
        st.map(carte, latitude="latitude", longitude="longitude", size="surface_reelle_bati", color="prix_m2")
    else:
        st.warning("Coordonnees hors limites (Lambert non converti).")
else:
    st.info("Pas de coordonnees disponibles.")

st.subheader("Detail des Transactions (dernieres)")
...
if cols_disponibles:
    ...
    if "valeur_fonciere" in df_affichage.columns:
        df_affichage["valeur_fonciere"] = df_affichage["valeur_fonciere"].apply(lambda x: f"{x:,.0f} EUR")
    if "prix_m2" in df_affichage.columns:
        df_affichage["prix_m2"] = df_affichage["prix_m2"].apply(lambda x: f"{x:,.0f} EUR/m2")

    st.dataframe(df_affichage[cols_disponibles], hide_index=True, width="stretch")
